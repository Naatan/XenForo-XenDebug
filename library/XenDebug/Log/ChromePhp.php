...
class XenDebug_Log_ChromePhp extends XenDebug_Log_Abstract
{
	...
	public function log($message, $type = self::TYPE_DEBUG, $level = 3)
	{
		...
		self::$_logSize += strlen($message);

		if (self::$_logSize > 6000) // Apart from nginx all web servers allow headers of 8K or more, let's give it 2K margin for additional headers.
		{
			if ( ! self::$_logSizeWarned)
			{
				ChromePhp::warn("WARNING: Log size exceeds 6K, further log messages are suppressed.");
				self::$_logSizeWarned = true;
			}
			return;
		}
		...
		switch ($type)
		{
			case XenDebug_Log::TYPE_WARNING:
				ChromePhp::warn($message);
				break;
			case XenDebug_Log::TYPE_ERROR:
			case XenDebug_Log::TYPE_EXCEPTION:
				ChromePhp::error($message);
				break;
			case XenDebug_Log::TYPE_INFO:
	...
	public function formatMessage($message, $type)
	{
		...
		return date('H:i:s') . ':' . substr((string) microtime(true),-2) . ' - ' . strtoupper($type) . ': ' . $message;
	}
